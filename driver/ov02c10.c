...
//
// Galaxy Book variant, based on the Linux 7.2.5 in-tree driver:
//  - accept a 26 MHz external clock and correct pixel rate, link frequency
//    and frame length for it (the register tables are for 19.2 MHz),
//  - report 180 degree, mirrored mounting on boards whose ACPI tables
//    don't say so,
//  - extend the analogue gain range with sensor digital gain and allow
//    a lower default frame rate for low light,
//  - implement get_selection() for the pixel array.
...
/*
 * On 26 MHz boards nothing re-times the PLL (OmniVision's 26 MHz PLL settings
 * are not public), so every internal clock runs 26 / 19.2 times faster than
 * the register tables intend: MIPI link, line time and frame rate. Instead of
 * guessing PLL values, keep the proven register tables, tell userspace the
 * real pixel rate and link frequency, and lengthen the frame to get 30 fps.
 */
static int rotation = -1;
module_param(rotation, int, 0444);
MODULE_PARM_DESC(rotation,
		 "Override mounting rotation (0 or 180). -1 = firmware/DMI quirk (default)");

static bool true_link_freq = true;
module_param(true_link_freq, bool, 0444);
MODULE_PARM_DESC(true_link_freq,
		 "Report the real MIPI link frequency on 26 MHz boards (default: true)");

static unsigned int gain_boost = 4;
module_param(gain_boost, uint, 0444);
MODULE_PARM_DESC(gain_boost,
		 "Extend analogue gain range by this much sensor digital gain, 1-16 (default: 4, 1 = off)");

static unsigned int analog_gain_max = 248;
module_param(analog_gain_max, uint, 0444);
MODULE_PARM_DESC(analog_gain_max,
		 "Highest analogue gain code (16 = 1x ... 248 = 15.5x) before switching to digital gain (default: 248)");

static unsigned int frame_rate = 30;
module_param(frame_rate, uint, 0444);
MODULE_PARM_DESC(frame_rate,
		 "Default frame rate, 5-30; lower allows longer exposure and less gain (default: 30)");

static int mirror = -1;
module_param(mirror, int, 0444);
MODULE_PARM_DESC(mirror,
		 "Sensor module is mirrored horizontally (0 or 1). -1 = DMI quirk (default)");
...
#define OV02C10_GAIN_BOOST_MAX		16
...
/*
 * Laptops with the sensor mounted upside-down that report a rotation of 0 in
 * the SSDB. Upstream keeps this list in ipu-bridge.
 */
static const struct dmi_system_id ov02c10_upside_down_dmi_ids[] = {
	{
...
struct ov02c10 {
	...
	/* External clock rate, 19.2 or 26 MHz */
	unsigned long mclk;
	/* Link frequency reported through V4L2_CID_LINK_FREQ */
	s64 link_freq;
	/* Module mounted mirrored: HFLIP control is inverted */
	bool hflip_invert;
};

/* Scale a value computed for the 19.2 MHz register tables to the real clock */
static u64 ov02c10_clk_scale(struct ov02c10 *ov02c10, u64 val)
{
	return div_u64(val * ov02c10->mclk + OV02C10_MCLK / 2, OV02C10_MCLK);
}

/* Default frame length: frame_rate fps regardless of lane count and clock */
static u32 ov02c10_vts_def(struct ov02c10 *ov02c10,
			   const struct ov02c10_mode *mode)
{
	u32 vts = ov02c10_clk_scale(ov02c10,
				    mode->vts_min * ov02c10->mipi_lanes);

	return min_t(u32, vts * 30 / clamp(frame_rate, 5U, 30U),
		     OV02C10_VTS_MAX);
}

/*
 * The analogue gain control covers the sensor's analogue range (1x - 15.5x)
 * and, past its top, continues with sensor digital gain. libcamera's software
 * ISP only drives V4L2_CID_ANALOGUE_GAIN, so this is what gives its AGC room in
 * low light. V4L2_CID_DIGITAL_GAIN still multiplies on top.
 */
static int ov02c10_write_gain(struct ov02c10 *ov02c10)
{
	u32 amax = clamp(analog_gain_max, (unsigned int)OV02C10_ANAL_GAIN_MIN,
			 (unsigned int)OV02C10_ANAL_GAIN_MAX);
	u32 again = ov02c10->analogue_gain->val;
	u64 dgain = ov02c10->digital_gain->val;
	int ret = 0;

	if (again > amax) {
		dgain = div_u64(dgain * again, amax);
		again = amax;
	}
	dgain = min_t(u64, dgain, OV02C10_DGTL_GAIN_MAX);

	cci_write(ov02c10->regmap, OV02C10_REG_ANALOG_GAIN, again << 4, &ret);
	cci_write(ov02c10->regmap, OV02C10_REG_DIGITAL_GAIN, dgain << 6, &ret);
...
static int ov02c10_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_DIGITAL_GAIN:
		ret = ov02c10_write_gain(ov02c10);
		break;
...
static int ov02c10_init_controls(struct ov02c10 *ov02c10)
{
	...
	ov02c10->link_freq = link_freq_menu_items[ov02c10->link_freq_index];
	if (true_link_freq)
		ov02c10->link_freq = ov02c10_clk_scale(ov02c10,
						       ov02c10->link_freq);

	/* Single entry menu holding the real link frequency */
	...
	pixel_rate = div_u64(ov02c10_clk_scale(ov02c10,
	...
	if (ov02c10->mclk != OV02C10_MCLK)
		dev_info(ov02c10->dev,
			 "%lu Hz clock: pixel rate %lld, link frequency %lld, vts %u\n",
			 ov02c10->mclk, pixel_rate, ov02c10->link_freq, vts_def);
	...
	ov02c10->analogue_gain =
		v4l2_ctrl_new_std(ctrl_hdlr, &ov02c10_ctrl_ops,
				  V4L2_CID_ANALOGUE_GAIN, OV02C10_ANAL_GAIN_MIN,
				  OV02C10_ANAL_GAIN_MAX *
				  clamp(gain_boost, 1U, OV02C10_GAIN_BOOST_MAX),
				  OV02C10_ANAL_GAIN_STEP,
				  OV02C10_ANAL_GAIN_DEFAULT);
	ov02c10->digital_gain =
		v4l2_ctrl_new_std(ctrl_hdlr, &ov02c10_ctrl_ops,
				  V4L2_CID_DIGITAL_GAIN, OV02C10_DGTL_GAIN_MIN,
				  OV02C10_DGTL_GAIN_MAX, OV02C10_DGTL_GAIN_STEP,
	...
	} else if (dmi_check_system(ov02c10_upside_down_dmi_ids) &&
		   (props.rotation == V4L2_FWNODE_PROPERTY_UNSET ||
		    props.rotation == 0)) {
		dev_info(ov02c10->dev, "sensor mounted upside-down (DMI quirk)\n");
		props.rotation = 180;
	}

	if (mirror == 0 || mirror == 1)
		ov02c10->hflip_invert = mirror;
	else
		ov02c10->hflip_invert = dmi_check_system(ov02c10_upside_down_dmi_ids);
	if (ov02c10->hflip_invert)
		dev_info(ov02c10->dev, "sensor mounted mirrored\n");
...
MODULE_DESCRIPTION("OmniVision OV02C10 sensor driver (Galaxy Book variant)");
MODULE_VERSION("1.0");
MODULE_LICENSE("GPL");
