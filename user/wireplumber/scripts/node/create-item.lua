...
function configProperties (node)
  ...
  -- Local override of the wireplumber 0.5.17 script (re-copy on upgrade):
  -- capture streams that don't declare a media type ("Stream/Input/Unknown",
  -- e.g. pipewiresrc) can't link to any source. Treat them as video capture.
  if media_class == "Stream/Input/Unknown" then
    properties ["media.type"] = "Video"
    log:info ("stream " .. tostring (properties ["node.name"]) ..
        " has no media type, assuming Video")
  end
